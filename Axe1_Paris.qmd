...
```{r}
loc<-map %>% filter(iso3 %in% c("MEX","VEN"))
...
```{r}
loc<-map %>% filter(iso3 %in% c("CHN","AUS"))
...
```{r}
loc<-map %>% filter(iso3 %in% c("ESP","DZA","TUN","LBY","TUR"))
...
loc<-map %>% filter(iso3 %in% c("ISL","UKR","SWE","IRN","GRL","EGY"))

mf_init(loc, expandBB = c(0,0,0,0))
mf_map(map, type="base", col="white",add=T)
...
loc<-map %>% filter(iso3 %in% c("CAN","MEX", "ISL"))
