...
# D. VERTICALE (Afrique-Méditerranée-Europe)
...
```{r}
don$afr <- don$mots_afr
don$afr<-gsub(", NA","",don$afr)
don$afr<-gsub("NA","",don$afr)
don$afr<-tolower(don$afr)
  don$afr<-gsub("ressources naturelles","ressources",don$afr)
  don$afr<-gsub("richesses naturelles","ressourcess",don$afr)
  don$afr<-gsub("richesse naturelle","ressources",don$afr)
...
## USSEIN
corp1<-corpus(x=don[don$lieu=="USSEIN - Kaolack - Sénégal",],text_field = "afr")
...
## PARIS
corp2<-corpus(x=don[don$lieu=="UPC - Paris - France",],text_field = "afr")
...
On reture de l'analyse le mot "*mer*" qui a été le plus cité par les deux groupes d'étudiants et ne crée pas de différence significative entre eux.

```{r}
don$med <- don$mots_med
...
don$med<-gsub("oliviers","olive",don$med)
don$med<-gsub("d'olive","olive",don$med)
don$med<-gsub("olivier","olive",don$med)
don$med<-gsub("méditerranée","",don$med)
don$med<-gsub("échanges","commerce",don$med)
don$med<-gsub("commerciaux","commerce",don$med)
don$med<-gsub("commerces","commerce",don$med)
don$med<-gsub("ressources","ressource",don$med)
don$med<-gsub("échange","commerce",don$med)
don$med<-gsub("echange","commerce",don$med)
don$med<-gsub("developpement","développement",don$med)
don$med<-gsub("chaud","chaleur",don$med)

#table(don$lieu)
## USSEIN
corp1<-corpus(x=don[don$lieu=="USSEIN - Kaolack - Sénégal",],text_field = "med")
toks1<- tokens(corp1,remove_punct = T)
toks1<-tokens_remove(toks1,c("des","les","pas","le","la","les","et","de","mer"))
dfmat1<-dfm(toks1)
tstat_freq1 <- textstat_frequency(dfmat1, n = 40)

## PARIS
corp2<-corpus(x=don[don$lieu=="UPC - Paris - France",],text_field = "med")
toks2<- tokens(corp2,remove_punct = T)
toks2<-tokens_remove(toks2,c("des","les","pas","le","la","les","et","de","mer"))
dfmat2<-dfm(toks2)
tstat_freq2 <- textstat_frequency(dfmat2, n = 40)
...
# Calculate keyness and determine Kaolack as target group
tstat_keyness <- textstat_keyness(dfmat, target = "USSEIN - Kaolack - Sénégal",measure = "chi2") %>% filter(chi2 < -2.71 | chi2 > 2.71)
...
```{r}
don$eur <- don$mots_eur
don$eur<-tolower(don$eur)
don$eur<-gsub("niveau de vie","niveau_de_vie",don$eur)
don$eur<-gsub("main d'oeuvre","travail",don$eur)
don$eur<-gsub("union européenne","union",don$eur)
don$eur<-gsub(" ue "," union ",don$eur)
...
don$eur<- gsub("développé","développement",don$eur)
don$eur<- gsub("espace schengen","schengen",don$eur)

#table(don$lieu)
## USSEIN
corp1<-corpus(x=don[don$lieu=="USSEIN - Kaolack - Sénégal",],text_field = "eur")
toks1<- tokens(corp1,remove_punct = T)
toks1<-tokens_remove(toks1,c("des","les","pas","le","la","les","et","de"))
dfmat1<-dfm(toks1)
tstat_freq1 <- textstat_frequency(dfmat1, n = 40)
...
## PARIS
corp2<-corpus(x=don[don$lieu=="UPC - Paris - France",],text_field = "eur")
toks2<- tokens(corp2,remove_punct = T)
toks2<-tokens_remove(toks2,c("des","les","pas","le","la","les","et","de"))
dfmat2<-dfm(toks2)
tstat_freq2 <- textstat_frequency(dfmat2, n = 40)
